...
def find_bin(n):
    result = []
    myQueue = deque()
    myQueue.append(1) # Starting with enqueueing 1 because if we want to print binary of 1, we just run the for loop once and dequeue into append result
    for i in range(n): # This works for edge case n = 0 because the for loop won't run and nothing gets appended to result
        result.append(str(myQueue.popleft())) # append the value that is dequeued for each iteration, given sequence from 1 to n
